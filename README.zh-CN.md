...
**回来。** 说「我回来了」或输入 `/agent-lark off`：agent 跑 `away off`——只关开关，群和 daemon 都留着。开关真正关掉之前，群里会先收到一张卡，说以后请回终端继续、群里再发的消息不会送达。任务结束时 agent 先问你群留不留：留 ⇒ `unbind`（群留在飞书里，这个项目下次开启远程模式时 agent 会把它提出来）；不留 ⇒ `unbind --dissolve`（解散群、忘掉记录）。只要那时远程模式还开着，两种情况都会先给群发一张卡说明本群即将与项目解绑；agent 帮你把项目换到另一个群时同理——旧群会被告知项目换到了哪，新群会收到和刚开启远程模式时一样的开启卡。远程模式只换通道，不降标准：不可逆动作仍要你明确批准，超时不算批准。

...
**Coming back.** Say "I'm back" or type `/agent-lark off`: the agent runs `away off` — the switch only; the group and the daemon stay. A card lands in the group first, before the switch actually flips, telling you to continue from the terminal and that messages sent there afterward will not be delivered. When the task is over the agent asks whether to keep the group: keep ⇒ `unbind` (the group stays in Feishu, and the next time this project turns remote mode on the agent offers it back); drop ⇒ `unbind --dissolve` (the group is dissolved and forgotten). Either way, if remote mode was still on, a card tells the group first that it is losing the project — the same courtesy applies when the agent switches you to a different group while remote mode is on: the old group is told where the project went, and the new one gets the same opening card. Remote mode changes the channel, not the standard: irreversible actions still need your explicit approval, and a timeout is not approval.
